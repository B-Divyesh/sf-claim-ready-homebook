...
function updateMetadata(): void {
  ...
  const canonicalPath = demoMode && route === 'inventory' ? '/demo' : route === 'notFound' ? '/404' : basePath(route);
  if (canonical) canonical.href = `${PRODUCT_ORIGIN}${canonicalPath}`;
